...
\end{center}
\caption{Выбор USB-устройства}\label{usbsel}
\end{figure}

Потом в системном трее виртуальной машины появляется всплывающее уведомление, о
том, что подключен съёмный диск (рис.\ref{usbmount}):

\newpage

\begin{figure}[!ht]
\begin{center}
\includegraphics[width=0.75\textwidth]{usbflash-mount.png}
\end{center}
\caption{Подключение съёмного диска}\label{usbmount}
\end{figure}

В ОС Linux нет разделения на диски как в ОС DOS и Windows. Все диски собираются
в единую древовидную файловую систему. Съёмные диски отображаются (монтируются)
в подкаталоги каталога (\verb|/var/run/media/<имя_пользователя>|). Например для
пользователя \verb|rpduser| флэшки следует искать в каталоге
\verb|/var/run/media/rpduser|.
Используя это уведомление, можно сразу открыть съёмный диск в диспетчере
файлов (рис.\ref{usbdolfin}):

\begin{figure}[!ht]
\begin{center}
\includegraphics[width=0.75\textwidth]{usbflash-dolfin.png}
\end{center}
\caption{Флеш-диск в диспетчере файлов Dolfin}\label{usbdolfin}
\end{figure}

Чтобы сделать скриншот, служит клавиша \verb|PrintScreen|. При этом открывается
окно программы KSnapshot, которая позволяет управлять скриншотами, сохранять их
в требуемом месте.

\subsection{Автоматическое получение методических указаний при помощи системы
контроля версий Git}

Методические указания по семинарам по курсу ОЦОС и лабораторным работам по
курсу РТУиС размещены в репозиториях сервера github.com и к ним подключена
система контроля версий Git. Для работы с репозиториями на Github.com  служит
команда \verb|git|. Чтобы закачать их в текущий каталог, можно в консоли
выполнить команду:

\begin{verbatim}
git clone https://github.com/ra3xdh/DSP-RPD
\end{verbatim}

При этом в текущем каталоге создаётся каталог DSP-RPD, в котором помещаются
скачанный методические указания. В этот каталог модно попасть потом из консоли
или из файлового менеджера. Чтобы выполнить обновление методических указаний,
нужно находясь в каталоге \verb|DSP-RPD| выполнить команду:

\begin{verbatim}
git pull
\end{verbatim}

При этом автоматически проверяется наличие обновлений на сервере github.com и
выкачивается последняя версия.

Чтобы получить лабораторные работы по курсу РТУиС нужно выполнить команду:

\begin{verbatim}
 git clone https://github.com/ra3xdh/RTUiS-labs
\end{verbatim}


Теперь методички можно просматривать, например из файлового менеджера Krusader
(рис. \ref{krus}). 

\begin{figure}[!ht]
\begin{center}
\includegraphics[width=0.75\textwidth]{krusader.png}
\end{center}
\caption{Файловый менеджер Krusader}\label{krus}
\end{figure}
...
Чтобы запустить Octave, нужно набрать в терминале \verb|octave|. Программа
загружается в интерактивном режиме и выдаёт в консоль приглашение. Чтобы
запустить скрипт Octave, нужно набрать в консоли:

\begin{verbatim}
 octave -qf имя_файла_со_скриптом
\end{verbatim}

Также можно запустить Octave с графическим интерфейсом. Для этого служит команда

\begin{verbatim}
 octave --force-gui
\end{verbatim}

Редактировать скрипты Octave можно, например, при помощи текстового
редактора Kwrite или Kate.\ref{oct}.

\begin{figure}
\begin{center}
 \includegraphics[width=0.8\textwidth]{kwrite-octave.png}
\end{center}
\caption{Octave в OpenSUSE 13.1 на VirtualBox.}\label{oct}
\end{figure}

Чтобы запустить Gnuplot служит команда \verb|gnuplot|. Мы попадаем в
интерактивный режим Gnuplot. Сохранять построенные графики в графические файлы
можно в графическом окне (рис.\ref{gnuplot}).

\begin{figure}
\begin{center}
 \includegraphics[width=0.8\textwidth]{gnuplot.png}
\end{center}
\caption{Gnuplot в OpenSUSE 13.1 на VirtualBox.}\label{gnuplot}
\end{figure}
